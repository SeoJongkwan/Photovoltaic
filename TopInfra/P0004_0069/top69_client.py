...
args.host = conf.get('server','container_host1')
args.port = conf.get('server','port1')
...
args.timezone = json.loads(conf.get('settings', 'timezone')) #이상감지 시간대:DB
...
def serving(rtu):
    ...
    if plant.empty:
        ...
        # plant_predict['diff_sum'] = plant_predict.copy()['diff_sum'][::-1].cumsum()#reverse dc_diff sum
        # plant_predict.to_csv("plant_predict.csv")
...
# @scheduler.scheduled_job('interval', minutes=args.interval, id='test')
def repeat():
