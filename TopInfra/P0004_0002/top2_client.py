...
# def time_check():
#     print("Scheduler Time Check: {}".format(datetime.now()))
#     if datetime.now().hour in args.timezone:
#         hour = datetime.now().hour 
#         min = datetime.now().minute + 1
#         if min > 59:
#             min = 0
#         print("Scheduler Start Time(HH:MM): {}:{} / Interval(MM): {}".format(hour, min, args.interval))
#         scheduler.add_job(start_runner, 'cron', hour=hour, minute=min)
#     else:
#         stop_runner()

# scheduler.add_job(time_check, 'interval', minutes=args.minutes)
